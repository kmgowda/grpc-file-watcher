...
void RunServer(int argc, char** argv) {
  ...
  if (!parseSecureCredentials(params, certs)) {
    ...
    std::shared_ptr< FileWatcherCertificateProvider > cert_provider =
            std::make_shared< FileWatcherCertificateProvider > (certs.key, certs.cert, certs.ca, 5);
    std::shared_ptr< grpc::ServerCredentials > creds = nullptr;
    grpc::experimental::TlsServerCredentialsOptions options( cert_provider );
